...
export const POST = async ({ params, request, locals }) => {
        ...
        let smfFound = matter.smf_detected;

        for (const file of files) {
                const path = `${matter.workspace_id}/${matter.id}/uploads/${Date.now()}_${file.name}`;
                ...
                if (docError || !document) {
                        throw error(500, docError?.message ?? 'Failed to register document');
                }
        ...
        const charges = await upsertCharges(supabase, matter.workspace_id, matter.id, extractedSnippets);
        await supabase
                .from('matters')
                .update({
                ...
                .eq('id', matter.id);
        await logAudit(supabase, matter.workspace_id, session.user.id, 'matter_documents_added', {
                matter_id: matter.id,
